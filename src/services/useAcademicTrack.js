...
export const useAcademicTrack = () => {
  ...
  const create = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["academic_track"] }),
  });

  const update = () =>
    useMutation({
      mutationFn: ({ id, data }) => apiService.update("academic_track", id, data),
      onSuccess: () =>
